...
export type ComponentGet = <T>(value: T) => T;
export type ComponentRender<RenderResult> = (
  (get: ComponentGet) => RenderResult
);
...
export default function define<RenderResult, Props extends string>(
  ...
  customElements.define(kebabify(name), class extends HTMLElement {
    ...
    constructor() {
      ...
      this.props = proxy(Object.fromEntries(
        currentProps.map((prop) => [prop, undefined]),
      ) as PropObject<Props>);
